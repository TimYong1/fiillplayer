...
/**
 * This is to load related from network so if not exists.
 * not used yet.
 */
...
public class LazyLoadManager extends IntentService {
    ...
    private static SoFetcher soFetcher=new SoFetcher(){
        ...
        public String getURL(String abi, String soVersion) {
            return String.format("https://raw.fiill.com/fiillPlayerLazyLoadFiles/master/%s/%s/so.zip", soVersion, abi);
        }
    };
    ...
    protected void onHandleIntent(@Nullable Intent intent) {
        ...
        //https://raw.githubusercontent.com/tcking/fiillPlayerLazyLoadFiles/master/0.1.17/armeabi/so.zip
        ...
        try {
            ...
            progress = -1;
            com.fiill.fiillplayer.utils.HttpRequest request = com.fiill.fiillplayer.utils.HttpRequest.get(url);
    ...
    private void error(String message) {
        ...
        if (lastFingprint != null) {
            StreamFiillPlayer player = PlayerManager.getInstance().getPlayerByFingerprint(lastFingprint);
            if (player != null) {
    ...
    private void installSo(File soDir) throws Exception {
        SoInstaller.installNativeLibraryPath(StreamFiillPlayer.class.getClassLoader(), soDir);
    ...
    public static File getPlayerRootDir(Context context) {
        return context.getDir("fiillPlayer2", Context.MODE_PRIVATE);
